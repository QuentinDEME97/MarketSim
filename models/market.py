...
from models.agent import Agent, Buyer, Seller
...
class Meeting(object):
    ...
    def __init__(
        self,
        buyer: Union[Agent, None] = None,
        seller: Union[Agent, None] = None,
        ...
        self.buyer = buyer
        self.seller = seller
        if self.buyer is None or self.seller is None:
            raise RuntimeError("Buyer or Seller should not be None")
        ...
        ask = max(
            self.seller.goal_prices[-1] - concession_size, self.seller.price_limit
        )
        ...
        if interaction_mode == "seller_asks_buyer_decides":
            ...
            if ask <= bid:
                ...
                if interaction_mode == "negotiate":
                    minimum = max(bid, self.seller.price_limit)
                    maximum = min(ask, self.buyer.price_limit)
                    if minimum <= maximum:
                    ...
                    if (
                        random() < 0.5
                    ):  # Just 50-50 willingness for now, regardless of spread
                        minimum = max(bid, self.seller.price_limit)
                        maximum = min(ask, self.buyer.price_limit)
                        if minimum <= maximum:
                            self.transaction_price = uniform(minimum, maximum + 1)
            ...
            while len(buyers_this_round) > 0 and len(sellers_this_round) > 0:
                ...
                if self.trim_participants == True:
                    ...
                    disqualified += [
                        ...
                        if x.goal_prices[-1] > max_buyer_price
                    ]
                    sellers_this_round = [
                        x
                        for x in sellers_this_round
                    ...
                    disqualified += [
                        ...
                        if x.goal_prices[-1] < min_seller_price
                    ]
                    buyers_this_round = [
                        x
                        for x in buyers_this_round
                ...
                while len(buyers_this_round) > 0 and len(sellers_this_round) > 0:
                    ...
                    if self.trim_participants == True:
                        ...
                        disqualified += [
                            x
                            for x in sellers_this_round
                            if max(x.goal_prices[-1] - PRICE_CONCESSION, x.price_limit)
                            > max_buyer_price
                        ]
                        sellers_this_round = [
                            x
                        ...
                        disqualified += [
                            x
                            for x in buyers_this_round
                            if min(x.goal_prices[-1] + PRICE_CONCESSION, x.price_limit)
                            < min_seller_price
                        ]
                        buyers_this_round = [
                            x
                    ...
                    # Create pairs
                    for i in range(
                        min(len(buyers_this_round), len(sellers_this_round))
                    ):
                        pair_has_been_tried = True
                        j = 0
                        no_possible_pairs = False
...
class Market(object):
        ...
        if initial_agents == None:
            if len(buyer_limits) > 0 and len(seller_limits) > 0:
                self.generate_agents(
                    buyer_limits=buyer_limits, seller_limits=seller_limits
                )
            else:
                self.generate_agents(
                    num_sellers=num_initial_sellers, num_buyers=num_initial_buyers
                )
        else:
    ...
    def generate_agents(
        self, num_buyers=None, num_sellers=None, buyer_limits=None, seller_limits=None
    ):
        new_agent_list = []
        agents_counter = 0
        if num_buyers != None:
    ...
    def new_session(
        ...
        for agent in self.agents_lists[-1]:
            if agent.type == "seller":
                if (
                    self.fluid_sellers == True
                    and expected_price != None
                    and expected_price < agent.price_limit
                ):
